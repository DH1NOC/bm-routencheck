...
| GPX-Import (Fallback für alles) | lokale Datei, Parsing per stdlib `xml.etree` (`<trkpt lat lon>`) | ✅ kein API-Risiko; Komoot bietet GPX-Export in jeder Tour an |
...
| R2 | `gmaps_link.py`, `komoot.py` und `gpx.py` mit Unit-Tests (echte URLs/Dateien aus R0 als Fixtures, inkl. Fehlerfälle: ÖPNV-Link, kaputter Blob, Consent-Wall, private Tour ohne share_token, leeres/kaputtes GPX) | pytest grün; jeder Fixture-Link liefert Wegpunkte + Modus bzw. Geometrie; Komoot-Geometrie deckt sich mit dem GPX-Referenz-Export derselben Tour |
| R3 | `routing.py`: OSRM-Route über alle Wegpunkte, Fallback-Kette, `Route`-Objekt (Komoot/GPX umgehen das Routing: Geometrie → direkt `Route`) | Karte des Tools neben Google-Route: Verlauf deckungsgleich bis auf dokumentierte Abweichungen; Distanz ±10 % zu Google |
