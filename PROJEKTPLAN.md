...
| FM-Regeln | **CTCSS ist der Pilotton — viele Relais öffnen NUR damit.** Ein angegebener Ton wird deshalb immer als Encode gesetzt (Gerät sendet ihn mit, in Codeplug UND Bericht); Decode default offen (`--ctcss-decode` setzt den Relais-Ton). Relais ohne CTCSS-Angabe brauchen ggf. 1750-Hz-Tonruf (Hinweis im Bericht). Ablage stets aus rx−tx berechnen, nie annehmen (NL-70cm nutzt +1,6 MHz); Bandbreite Codeplug default 12,5 kHz, `--bandbreite 25` global (kein Raster in den Daten). TG9/Slot-0/Colorcode-Regeln gelten NICHT für FM |
